...
export default function StockAddPage() {
  ...
  const onChangeSrc = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setError("두번째 파일(추가할 데이터) 파싱 중 오류가 발생했습니다.");
      }
    },
    [base],
  );
  ...
  const merged = useMemo(() => {
    ...
    const rows = base.rows.map((row) => {
      ...
      if (baseQtyKey && chosenQtyKey) {
        ...
        const incomingNum = parseNumber(incoming);

        if (incomingNum !== null) {
          const baseNum = currentNum ?? 0;
          const newTotal = baseNum + incomingNum;

          if (currentNum === null || newTotal !== baseNum) {
            cloned[baseQtyKey] = newTotal;
      ...
      // 상품 매장 진열 위치: 덮어쓰기 동작은 기존 페이지와 동일하게 유지
  ...
  const downloadExcel = useCallback(() => {
    ...
    XLSX.writeFile(wb, `added_${base.filename.replace(/\.(xlsx|xls|csv)$/i, "")}.xlsx`);
  }, [base, merged]);

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-semibold mb-4">재고 추가하기</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="border rounded-lg p-4">
          <h2 className="font-medium mb-2">첫번째 파일 (기준)</h2>
          <div className="flex items-center gap-3">
            <input id="base-file-add" type="file" accept=".xlsx,.xls,.csv" onChange={onChangeBase} className="sr-only" />
            <label
              htmlFor="base-file-add"
          ...
          {base && (
            <div className="mt-3 text-sm text-gray-700">
              <div className="truncate">
                <span className="text-gray-500">파일명:</span> {base.filename}
              </div>
              <div className="text-gray-500">
                행 수: <span className="text-gray-800">{base.rows.length}</span> · 열 수:{" "}
                <span className="text-gray-800">{base.headers.length}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="border rounded-lg p-4">
          <h2 className="font-medium mb-2">두번째 파일 (추가할 데이터)</h2>
          <div className="flex items-center gap-3">
            <input id="overlay-file-add" type="file" accept=".xlsx,.xls,.csv" onChange={onChangeSrc} className="sr-only" />
            <label
              htmlFor="overlay-file-add"
              className={`inline-flex items-center gap-2 px-3 py-2 rounded text-sm font-medium cursor-pointer transition-colors ${
          ...
          {src && (
            <div className="mt-3 text-sm text-gray-700">
              <div className="truncate">
                <span className="text-gray-500">파일명:</span> {src.filename}
              </div>
              <div className="text-gray-500">
                행 수: <span className="text-gray-800">{src.rows.length}</span> · 열 수:{" "}
                <span className="text-gray-800">{src.headers.length}</span>
              </div>
            </div>
          )}
        </div>
      ...
      {base && (
        <>
          ...
          <div
            ...
            className="table-scroll border rounded-lg w-full overflow-x-scroll overflow-y-hidden mb-2"
          >
          ...
          <div
            ...
            className="table-scroll border rounded-lg w-full overflow-x-scroll overflow-y-auto"
          >
